...
type DatastarFragment = { Data: string; Merge: bool }
module DatastarFragment =
    let serialize fragment =
        seq {
            "event: datastar-fragment"
            if fragment.Merge then "data: merge upsert_attributes"
            $"data: fragment {fragment.Data}"
            "";""
        } |> String.concat "\n"

    let serializeToBytes = serialize >> System.Text.Encoding.UTF8.GetBytes
